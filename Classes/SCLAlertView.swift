...
open class SCLAlertView: UIViewController {
	...
	open var buttonsLayout: SCLButtonsLayout = .vertical
	...
	override open func touchesEnded(_ touches:Set<UITouch>, with event:UIEvent?) {
		if let event = event, let touches = event.touches(for: view), touches.count > 0 {
	...
	open func showTitle(_ title: String, subTitle: String, duration: TimeInterval?, completeText: String?, cancelText: String? = nil, style: SCLAlertViewStyle, icon: UIImage?) -> SCLAlertViewResponder {
		...
		let cancelSelector = #selector(SCLAlertView.cancelView)

		if buttonsLayout == .horizontal {
			if cancelText != nil {
				let _ = addButton(cancelText!, target:self, selector: cancelSelector)
			}
		}
		// Done button
		if completeText != nil {
			let _ = addButton(completeText!, target:self, selector:#selector(SCLAlertView.hideView))
		}
